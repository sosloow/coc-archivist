defmodule CocArchivistWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: CocArchivistWeb.Gettext
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-gray-700/10 ring-gray-700/10 relative hidden rounded-2xl bg-gray-800 p-14 shadow-lg ring-1 transition"
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <.icon name="hero-x-mark-solid" class="h-5 w-5 text-gray-100" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
    ...
    <div>
      <label class="flex items-center gap-4 text-sm leading-6 text-gray-300">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-gray-600 bg-gray-700 text-emerald-400 focus:ring-emerald-500"
          {@rest}
        />
        {@label}
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-gray-600 bg-gray-700 text-gray-100 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "mt-2 block w-full rounded-lg text-gray-100 focus:ring-0 sm:text-sm sm:leading-6 min-h-[6rem]",
          @errors == [] && "border-gray-600 bg-gray-700 focus:border-emerald-500",
  ...
  def table(assigns) do
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        <thead class="text-sm text-left leading-6 text-gray-300">
          <tr>
        ...
        <tbody
          ...
          class="relative divide-y divide-gray-700 border-t border-gray-700 text-sm leading-6 text-gray-100"
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-gray-800">
            <td
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-gray-700">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          <dt class="w-1/4 flex-none text-gray-300">{item.title}</dt>
          <dd class="text-gray-100">{render_slot(item)}</dd>
        </div>
      </dl>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(CocArchivistWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(CocArchivistWeb.Gettext, "errors", msg, opts)
    end
  end
